...
class CMLProductUpdateProcessor extends modObjectUpdateProcessor {
    ...
    public function beforeSet() {
        ...
        if (empty($name)) {
            $this->addFieldError('name',$this->modx->lexicon('commerce_multilang.err.product_name_ns'));
        } else if ($this->modx->getCount($this->classKey, array('name' => $name)) && ($this->object->name != $name)) {
            $this->addFieldError('name',$this->modx->lexicon('commerce_multilang.err.product_name_ae'));
        }
    ...
    public function afterSave() {
        ...
        $productLanguages = $this->modx->getCollection('CMLProductLanguage',[
            'product_id'    => $this->object->get('id')
        ]);
