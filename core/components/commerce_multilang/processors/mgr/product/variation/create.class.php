 ...
 */
class CMLProductChildCreateProcessor extends modObjectCreateProcessor {
    ...
    protected function loadParentProduct() {
        $this->parentObj = $this->modx->getObject($this->classKey,array('CMLProduct.id' => $this->getProperty('parent')));
        $this->parentLangs = $this->modx->getCollection('CMLProductLanguage',array(
    ...
    public function beforeSet() {
        ...
        // Duplicate parent values into new CMLProduct object (Not the ID!).
    ...
    public function beforeSave() {
        ...
        if (empty($sku)) {
            ...
            $this->addFieldError('sku',$this->modx->lexicon('commerce_multilang.err.product_type_sku_ae'));
        }
    ...
    public function afterSave() {
        ...
        $items = $this->modx->getCollection('CMLProductData');
        $productData->set('position', count($items));
        $productData->save();

        $productImage = $this->modx->newObject('CMLProductImage');
        ...
        if($productImage) {
            foreach($this->parentLangs as $lang) {
                $imageLanguage = $this->modx->newObject('CMLProductImageLanguage');
