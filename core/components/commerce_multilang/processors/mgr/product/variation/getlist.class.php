 ...
 */
class CMLProductChildGetListProcessor extends modObjectGetListProcessor {
    ...
    /*protected function loadExtraColumns() {
        ...
        $parentData = $this->modx->getObject('CMLProductData',array(
            'product_id'    =>  $this->getProperty('product_id')
        ));
        if($parentData) {
            $this->productType = $this->modx->getObject('CMLProductType', array(
                'id' => $parentData->get('type')
            ));

            //$this->modx->log(1, print_r($this->productType->toArray(), true));
            if ($this->productType) {
                $cols = $this->modx->getCollection('CMLProductVariation', array(
    ...
    public function prepareRow(xPDOObject $object) {
        ...
        $variations = $this->modx->getCollection('CMLAssignedVariation',array(
            'product_id'    =>  $row['id'],
...
return 'CMLProductChildGetListProcessor';